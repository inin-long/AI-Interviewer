...
public class DashboardController {
    ...
    private void refreshTasks() {
        var tasks = taskService.list(sessionState.requireCurrentUser().id());
        long pending = tasks.stream().filter(task -> task.getStatus() == BackgroundTaskStatus.PENDING).count();
        long running = tasks.stream().filter(task -> task.getStatus() == BackgroundTaskStatus.RUNNING).count();
        long failed = tasks.stream().filter(task -> task.getStatus() == BackgroundTaskStatus.FAILED).count();
        taskSummaryLabel.setText("等待 " + pending + " · 执行中 " + running + " · 失败 " + failed);
        if (tasks.isEmpty()) {
            latestTaskLabel.setText("暂无后台任务。上传知识文档后，可在这里查看处理状态。");
            return;
        }
        BackgroundTaskEntity latest = tasks.getFirst();
        latestTaskLabel.setText("最近任务：" + typeText(latest.getTaskType()) + " · " + statusText(latest));
    }
    ...
    private String statusText(BackgroundTaskEntity task) {
        return switch (task.getStatus()) {
            case PENDING -> "等待执行";
            case RUNNING -> "执行中（第 " + task.getAttemptCount() + " 次）";
            case SUCCESS -> "已完成";
            case FAILED -> "失败：" + (task.getErrorMessage() == null ? "未知错误" : task.getErrorMessage());
        };
    }
}

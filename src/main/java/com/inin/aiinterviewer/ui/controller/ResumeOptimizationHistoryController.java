...
@Component
@Scope("prototype")
public class ResumeOptimizationHistoryController implements ContextAwareController<Object> {
    ...
    @FXML
    private ListView<ResumeOptimizationDto> historyList;
    ...
    public void initialize() {
        ...
        List<ResumeOptimizationDto> list = careerPlanningService.listOptimizations(userId);

        historyList.setCellFactory(l -> new ListCell<>() {
            @Override
            protected void updateItem(ResumeOptimizationDto item, boolean empty) {
        ...
        historyList.setOnMouseClicked(e -> {
            if (e.getClickCount() == 2) {
                ResumeOptimizationDto selected = historyList.getSelectionModel().getSelectedItem();
                if (selected != null && selected.optimizedText() != null) {
                    showDetail(selected);
                }
            }
        });
    ...
    private void showDetail(ResumeOptimizationDto opt) {
        ...
        md.setMarkdown(opt.optimizedText());

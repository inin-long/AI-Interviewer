...
public class KnowledgeDetailController implements ContextAwareController<Long> {
    ...
    @FXML private AppSelect<String> categoryBox;
    ...
    private void saveChanges() {
        try {
            ...
            if (parsedChunks.size() != chunkIds.size()) {
                com.inin.aiinterviewer.ui.component.AppDialogs.showMessage(
                        chunksArea.getScene() == null ? null : chunksArea.getScene().getWindow(),
                        "片段数量不匹配",
                        "片段数量已变化（原" + chunkIds.size() + " → 现" + parsedChunks.size()
                                + "）",
                        "将按顺序匹配更新前" + Math.min(parsedChunks.size(), chunkIds.size())
                                + "个片段，超出部分将被忽略。",
                        com.inin.aiinterviewer.ui.component.AppDialog.Tone.WARNING);
            }

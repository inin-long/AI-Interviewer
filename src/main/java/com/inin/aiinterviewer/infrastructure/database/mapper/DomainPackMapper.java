...
public interface DomainPackMapper {
    ...
    @Insert("""
            INSERT INTO domain_pack(id, role_code, industry_code, display_name, version,
                                    content_json, enabled, create_time, update_time)
            VALUES(#{id}, #{roleCode}, #{industryCode}, #{displayName}, #{version},
                   #{contentJson}, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    ...
    @Update("UPDATE domain_pack SET enabled = 0, update_time = CURRENT_TIMESTAMP")
    int disableAll();

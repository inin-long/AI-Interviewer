...
- `candidate_profile_get` Agent Tool 只读取当前会话冻结的画像快照，为提问和追问提供受控上下文
- SQLite 持久化后台任务队列，支持并发原子领取、固定次数重试、失败留痕与启动恢复
- 知识文档上传后异步执行解析、切片、Embedding 和索引，不阻塞 JavaFX 页面
- 简历上传后由可恢复 `RESUME_PARSE` 任务执行解析，并在列表页显示重试/完成状态
- 候选人画像由可恢复 `PROFILE_GENERATE` 任务生成，AI 格式错误进入统一重试与失败记录
- 候选人画像提供独立列表和编辑确认子页面，可按关联简历集中查看状态
- 面试方案只能选择当前账户已确认的画像，并自动保持画像与关联简历一致
- 创建面试会话时冻结方案与候选人画像快照，后续编辑不会改变历史面试上下文
- 面试方案支持多选已完成索引的知识文档，会话创建时冻结本场知识范围
- `knowledge_search` 使用 Lucene 文档过滤查询，只召回当前会话选中的资料且不回退到全库
- AI 问题按消息保存知识片段引用；面试工作区、历史问答和 Markdown 报告均可追溯来源
- 面试工作区与历史记录使用统一的 `Qn` 问答卡片文档流，报告引用可直接定位到对应题目
- 最终回答与报告生成解耦保存；评分失败会持久化失败原因，并支持不重复回答的幂等重试
- 面试记录搜索、状态筛选、继续面试，以及列表与完整问答详情页分离
- 报告从面试记录进入，并可在报告与对应完整问答之间导航
- 首页后台任务状态摘要，展示等待、执行中、失败和最近一次任务
- 统一后台任务中心采用列表与详情分离，失败任务可在详情页手动重新排队

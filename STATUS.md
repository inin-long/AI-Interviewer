...
- [ ] Phase S1-6：增强报告、分支复盘与训练闭环
  ...
  - [ ] Session Branch 与 Branch Comparison
  - [ ] 专项训练建议与方案创建

最近一次 S1 验证（2026-07-17）：S1-1 至 S1-5 全部完成，S1-6 已完成 15 章节增强报告与 Evidence 问答跳转；DomainPack、Claim/Evidence Ledger、结构化追问、重要回答逻辑拆解、逐轮能力证据、证据不足语义、评分置信度、报告证据来源、相关主张/周期调度、`POTENTIAL → CLARIFIED → RESOLVED/CONFIRMED_CONFLICT` 生命周期、中性澄清、重要主张延迟验证、降级交叉验证、调度幂等完成与用户隔离、证据驱动压力阶梯、连续/重复施压降级、攻击性话术替换、结构化场景状态、决策与事件因果关联、变量时间线重放校验、候选人决策驱动的动态事件、完整内置场景模板、方案级模式/Persona/压力/严格度/场景比例配置、冻结快照与题量预算驱动的场景调度、六类 Persona 措辞与评分隔离、逐题质量门、单次重写与可信模板降级、七类协作行为证据、面试状态可视化、判断来源 ID/Q 编号、证据侧栏跳转、隐藏信息隔离、问题成功落库后推进、导演失败自动退出场景、结构化输出修复/降级、Checkpoint V1/V2.0/V2.1/V2.2/V2.3/V2.4/V2.5/V2.6/V2.7→V2.8 升级和本地完整 TestFX 业务流程测试通过；数据库迁移基线为 V20。
...
最近一次稳定覆盖率基线（2026-07-17）：使用 `D:\Libs\Java\jdk-21.0.2` 执行 `mvnw clean verify` 通过；Surefire 136 项（134 通过、2 个真实测试跳过），Failsafe 2 项（本地 TestFX 通过、真实 TestFX 跳过）。总体行覆盖率 82.7%、分支覆盖率 57.6%，均高于 70%/45% 门槛。真实 Provider 测试不计入稳定覆盖率基线。
